#!/usr/bin/env python3
"""
Ensure Hub Rank assets are consistent:
- Prefer CSV -> JSON
- If CSV empty and JSON exists -> JSON -> CSV
- If both missing/empty -> write small placeholder so downloads never 404
"""
...
def csv_has_rows(p: Path) -> bool:
    ...
    try:
        df = pd.read_csv(p)
        return df.shape[0] > 0 and any(pd.api.types.is_numeric_dtype(df[c]) for c in df.columns)
    except Exception:
        return False
...
def write_json_from_df(df: pd.DataFrame):
    # heuristik kolom
    label_pref = ["iata", "airport", "name", "node"]
    ...
    if not label or not val:
        raise ValueError("Cannot determine label/value columns")
    ...
    fig = {
        ...
        "layout": {
            ...
            "yaxis": {"title": val, "automargin": True},
            "height": 520
        }
    }
...
def write_placeholder():
    ...
    # sinkronkan JSON juga
    write_json_from_df(pd.read_csv(CSV))
    print("[warn] wrote placeholder CSV/JSON so downloads are not empty")

def main():
    ASSETS.mkdir(parents=True, exist_ok=True)

    csv_ok = csv_has_rows(CSV)
    json_ok, x, y = json_has_xy(JSONP)
