...
def _pick_series() -> SeriesPack | None:
    ...
    for csv_path in sorted(PUBLISH_DIR.glob("*.csv")):
        try:
            ...
            if not date_col:
                continue
            value_col = next((c for c in df.columns if c!=date_col and pd.api.types.is_numeric_dtype(df[c])), None)
...
def _seasonal_index(y: pd.Series) -> pd.Series:
    ...
    df = y.to_frame("y"); df["m"] = df.index.month
...
def _fit_forecast(y: pd.Series, horizon: int = 6):
    ...
    base = y.to_frame("y"); base["m"] = base.index.month
    ...
    anomalies = y[rz.abs() >= 3.5]
    return fitted, forecast, si, anomalies

def _serialize(pack: SeriesPack | None):
    now = datetime.now(timezone.utc).isoformat()
    if pack is None:
        return {"status":"no_input","generated_at":now,"series":[],"fitted":[],"forecast":[],"anomalies":[],"seasonality":[],"meta":{"note":"No suitable dataset found in publish/; placeholder written."}}
    y = pack.y
    ...
    return {"status":"ok","generated_at":now,"source":pack.name,"series":to_pairs(y),"fitted":to_pairs(fitted),"forecast":to_pairs(forecast),"anomalies":to_pairs(anomalies),"seasonality":[{"m":int(i.month),"si":float(v)} for i,v in si.dropna().items()]}
